...
public class YOLOView: UIView, VideoCaptureDelegate {
  ...
  func showBoxes(predictions: YOLOResult) {
    ...
    func applyHardCodedDistanceCorrection(_ rawDistanceM: Float) -> Float {
      ...
      // Hard-coded distance correction:
      // > 6 ft (72 in): halve the distance
      // 3 ft – 5 ft (36–60 in): multiply by 0.75
      // < 3 ft: no correction
      if rawInches > 72.0 {
        return (rawInches * 0.5) / 39.37007874
      } else if rawInches >= 36.0 && rawInches <= 60.0 {
        return (rawInches * 0.667) / 39.37007874
      }
    ...
    if UIDevice.current.orientation == .portrait {
      ...
      for i in 0..<boundingBoxViews.count {
        if i < resultCount && i < 50 {
          ...
          if let suffix = measurementSuffix(for: predictions.boxes[i]) {
            label = "\(label) \(suffix)"
          }
